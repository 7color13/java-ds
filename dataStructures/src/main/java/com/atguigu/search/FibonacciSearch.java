...
public class FibonacciSearch {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(fibSearch(arr,899));
    }

    //因为我们mid = low+F(k-1)-1,需要使用到斐波那契数列，因此需要构建一个斐波那契数列
    //非递归方法得到斐波那契数列
    ...
    public static int fibSearch(int[] a,int key){
        ...
        while (low<=high){
            ...
            if (key<temp[mid]){  //我们应该向数组的前面查找（左边）
                high = mid -1;
                k--; //说明：全部元素=前面的元素+后边元素；f[k] = f[k-1]+f[k-2];
                //因为前面有f[k-1]个元素，所以可以继续拆分f[k-1] = f[k-2]+f[k-3]
                //即在f[k-1] 的前面继续查找 k--,即下次循环时mid = f[k-1-1]-1;
